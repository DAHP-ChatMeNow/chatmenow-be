...
exports.getPremiumConfig = async (req, res) => {
  try {
    const config = await adminService.getPremiumConfig();
...
exports.updatePremiumConfig = async (req, res) => {
  try {
    const config = await adminService.updatePremiumConfig(req.body || {});

    res.status(200).json({
      success: true,
      message: "Đã cập nhật cấu hình Premium",
      config,
    });
  } catch (error) {
    if (error.statusCode) {
