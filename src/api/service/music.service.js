...
const JAMENDO_CLIENT_ID =
    process.env.JAMENDO_CLIENT_ID && process.env.JAMENDO_CLIENT_ID !== "your_jamendo_client_id_here"
...
class MusicService {
    ...
    async search(query = "", limit = 20) {
        ...
        try {
            ...
            try {
                const fallback = await Music.find({}).sort({ cachedAt: -1 }).limit(limit).lean();
    ...
    async getPopular(limit = 20) {
        ...
        try {
            ...
            console.log("[MusicService] Fetching popular from Jamendo:", url);
            const json = await this._fetchJson(url);
            const tracks = (json.results || []).map((t) => this._mapJamendoTrack(t));
